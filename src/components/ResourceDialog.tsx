      ...
      case "Videos": {
        return (
          <div style={{ flexDirection: "row", display: "flex" }}>
            <div style={{ flex: 2 }}>
              <img
                src={resource.thumbnail_img}
                style={{ borderRadius: 3 }}
                width="50px"
              />
            </div>
            <div style={{ flex: 7, paddingTop: 5 }}>{resource.title}</div>
          </div>
        );
      }
      case "Notes": {
      ...
      case "Snippets": {
        return (
          <div>
            ...
            <div
              className={classes.markdownResult}
...
const TopicsTab = ({
  ...
  const getTopicResources = async (resourceType) => {
    ...
    switch (resourceType) {
      ...
      case "Snippets": {
        const snippets = await getArticleSnippets(
          selectedTopic.article_snippets
        );
  ...
  console.log("TOPIC HUH: ", selectedTopic);
    ...
    <div>
          ...
          {topics.keys.map((topicKey) => {
            return (
              <div
                ...
                key={topicKey}
                onClick={() => {
                  onSelectTopic(topics.data[topicKey]);
                }}
              >
      ...
      {menuScreen === "RESOURCE_TYPES" && (
        <>
          ...
          {selectedTopic?.article_snippets.length > 0 && (
            <div
              className={classes.menuItem}
              onClick={gotoResources}
              data-resource="Snippets"
            >
              <div>Snippets</div>
              <div>
                <i className="fa fa-chevron-right" />
          ...
          >
            <div>
              <i
                className="fa fa-arrow-left"
                style={{ cursor: "pointer", fontSize: 14 }}
                onClick={gotoResourceTypes}
              />
              <span style={{ fontSize: 14, marginLeft: 10 }}>
                {selectedTopic.title} / {resourceType}
              </span>
            </div>
          </div>
          <div style={{ maxHeight: 200, overflowY: "auto", marginTop: 3 }}>
            {loadingResources && <LoadingResults classes={classes} />}
            {!loadingResources &&
              resources &&
    ...
    if (resource.result_type === "video") {
      ...
      return (
        <>
          ...
          <div
            className={classes.markdownResult}
            dangerouslySetInnerHTML={{ __html: marked(resource.title) }}
          ></div>
        </>
      );
    } else {
      return (
        <>
          <ResultTypeIcon type={resource.result_type} />
          <span className={classes.resTitle}>{resource.title}</span>
        </>
      );
    }
  };
